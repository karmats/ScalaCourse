...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val m = w.toLowerCase() groupBy ((element: Char) => element)
    val l = m.toList map (stringChar => (stringChar._1, stringChar._2.length()))
    l.sorted
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    def toWord(w: Word, sent: Sentence): Word = sent match {
      case Nil => w
      case head :: tail =>
        toWord(w.concat(head), tail)
    }
    wordOccurrences(toWord("", s))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y.toMap.foldLeft(x.toMap)((map, occ) => {
      val freq = map(occ._1) - occ._2
      if (freq <= 0) map - occ._1
      else map.updated(occ._1, freq)
    })).toList.sorted
  }
